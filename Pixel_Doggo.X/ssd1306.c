...
void ssd1306_drawLetter(unsigned char x_start, unsigned char y_start, char letter) {
    int color;
    int i;
    int j;
    //unsigned short column;
    for (i=0; i<5; i++) {
        //column = ASCII[letter-0x20][i];
        //column = 0x5f;
        for (j=0; j<8; j++) {
            color = ASCII[letter-0x20][i]>>j & 1;
            ssd1306_drawPixel(x_start+i, y_start+j, color);
        }
    }
}

void ssd1306_drawString(unsigned char string[], unsigned char line, unsigned char offset) {
    int s=0;
    while (string[s]!=0) {
        ssd1306_drawLetter((s+offset)*5,line*8,string[s]);
        s++;
    }
}